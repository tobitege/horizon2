...
--
-- Dependencies: `pl.plutils`, `pl.tablex`, `pl.class`
-- @classmod pl.List
-- @pragma nostrip
...
local array_tostring,split,assert_arg,function_arg = plutils.array_tostring,plutils.split,plutils.assert_arg,plutils.function_arg
local normalize_slice = tablex._normalize_slice

-- metatable for our list and map objects has already been defined..
local Multimap = plutils.stdmt.MultiMap
local List = plutils.stdmt.List
...
-- @usage for x in iterate {1,10,22,55} do print(x,',') end ==> 1,10,22,55
-- @usage for ch in iterate 'help' do do print(ch,' ') end ==> h e l p
function List.iterate(seq)
    ...
    elseif type(seq) == 'function' then
        return seq
    elseif type(seq) == 'userdata' and type(seq) == 'file' then
        return seq:lines()
    end
end
iter = List.iterate
